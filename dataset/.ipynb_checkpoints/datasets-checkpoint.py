...
class WYDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[384, 384], scale_factor=0.,
        ...
        self.dataset = dataset
        self.anno_list = glob.glob(os.path.join(root, 'png_label', dataset, '*.png'))
        self.anno_list.sort()
        self.anno_list = self.anno_list[:20]
        self.number_samples = len(self.anno_list)
    ...
    def __getitem__(self, index):
        ...
        im_name = anno_path.split('/')[-1].replace('png', 'jpg')
        ...
        if type(im) == None:
            print('Error in read file ', im_path)
        is_rotated = 0
        if 'test_no_label' not in self.dataset:
            parsing_anno = read_mask(anno_path)
            if type(parsing_anno) == None:
                print('Error in read file ', anno_path)
            ...
            if 'test' in self.dataset:
                if im.size(2) > im.size(1):
                    im = im.transpose(1,2)
                    parsing_anno = parsing_anno.transpose(1,2)
                    is_rotated = 1
...
#                 rf = self.rotation_factor
#                 s = s * np.clip(np.random.randn() * sf + 1, 1 - sf, 1 + sf)
#                 r = np.clip(np.random.randint(4) * rf, 0, 360) if random.random() <= 0.5 else 0
