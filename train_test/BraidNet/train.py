...
from utils.criterion import CriterionAll2
...
def adjust_learning_rate_parsing(optimizer, epoch):
    times = math.pow(0.25, epoch)
    lr = times * 0.05
    optimizer.param_groups[0]['lr'] = lr*0.1 # 1e-3
    for i in range(1,len( optimizer.param_groups)):
        optimizer.param_groups[i]['lr'] = lr
    return lr
...
def main():
    ...
    for part in deeplab.path_list:
        params_list.append({'params': part.parameters()})
    print ('len(params_list)',len(params_list))
    optimizer = torch.optim.SGD(params_list, lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
