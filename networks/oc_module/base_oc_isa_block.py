...
class _SelfAttentionBlock(nn.Module):
    ...
    '''
    def __init__(self, in_channels, key_channels, value_channels, out_channels=None, scale=1, ph = 8, pw = 8):
    ...
    def forward(self, x):
        ...
        x = x.view(N, C, Qh, self.ph, Qw, self.pw)
        #print("x.size(): ", x.size())
        
        # Long−range Attention
        x = x.permute(0, 3, 5, 1, 2, 4)
        #print("x.size(): ", x.size())
        x = x.reshape(N*self.ph*self.pw, C, Qh, Qw)
        #print("x.size(): ", x.size())
        x = self.selfattention(x)
        #print("x.size(): ", x.size())
        x = x.view(N, self.ph, self.pw, C, Qh, Qw)
        #print("x.size(): ", x.size())
        
        # Short−range Attention
        x = x.permute(0, 4, 5, 3, 1, 2)
        #print("x.size(): ", x.size())
        x = x.reshape(N*Qh*Qw, C, self.ph, self.pw)
        #print("x.size(): ", x.size())
        x = self.selfattention(x)
        #print("x.size(): ", x.size())
        x = x.view(N, Qh, Qw, C, self.ph, self.pw)
        #print("x.size(): ", x.size())
