...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        #zm# self.edge_layer = Edge_Module()
    ...
    def forward(self, x):
        ...
        x3 = self.layer2(x2)
        x4 = self.layer3(x3)
        x5 = self.layer4(x4)#1/16
        x = self.layer5(x5)
        #zm# edge,edge_fea = self.edge_layer(x2,x3,x4) 
        seg1 = self.layer6(x,x2) #zm# seg1,x = self.layer6(x,x2)
        #zm# x = torch.cat([x, edge_fea], dim=1)
        #zm# seg2 = self.layer7(x)
